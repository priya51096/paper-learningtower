...
```{r plotly-maps, fig.cap="Interactive maps showing the difference in average math, reading, and science scores between girls and boys across the world, positive values indicate the girls average to be higher than boys average. Mousing over the plots shows the country and score.  The reading scores are positive for every country, indicating that girls outperform boys across the globe in 2018.", fig.pos="H", fig.height=12, fig.width=18, out.width="100%", layout="l-body", include=knitr::is_html_output(), eval=knitr::is_html_output()}
...
```{r anim-plot, fig.cap = "Animation of math and reading scores over time, with selected countries labelled. Australia has a quite stable high scores over the full time period. There is quite a lot of moving of scores between years, perhaps more among lower scoring countries.", eval = knitr::is_html_output(), fig.width = 10, fig.height = 10, out.width="100%", layout="l-body-outset"}
...
We would like to express our sincere gratitude to the developers of the [rjtools](https://github.com/rjournal/rjtools) [@rjtools] package it is through their cumulative efforts that the developers of \CRANpkg{learningtower} package are able to compose and submit this R journal.
